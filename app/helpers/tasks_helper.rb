module TasksHelper
  ...
  def watch_button task
    if (task.watcher_users.ids.include?(current_user.id))
      ...
      button_to 'Watch', task_tasks_watchers_path(task, { tasks_watcher: { watcher_id: current_user.id, watcher_type: 'User' }}), class: 'ml-3 btn btn-dark', method: :post, remote: true
    end
  end
